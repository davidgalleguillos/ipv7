...
use transport::handshake::{HandshakeResponse, HandshakeSession};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mode = if args.len() < 2 { "--listen" } else { &args[1] };
    ...
    if mode == "--listen" {
        ...
        tracing::info!(
            "[📡] Escuchando activamente en el Subpuerto IPv7: {}",
            relay.bound_port
        );
                    ...
                    match Ipv7Packet::from_bytes(&buf[..amt]) {
                        Ok(packet) => {
                            ...
                            if packet.verify_origin_signature() {
                                let _ = tx_clone
                                    .send(TuiEvent::LogMsg(
                                        "    [✓] Firma ED25519 de Origen AUTENTICADA.".to_string(),
                                    ))
                                    .await;

                                // AUTO-DESCUBRIMIENTO: Añadimos a quien sea que nos hable a nuestro Kademlia DHT (NAT Punching pasivo)
                                dht_clone
                                    .register_node(packet.source_id, src.to_string())
                                    .await;

                                // Evaluar qué tipo de Payload es
                                if let Ok(dht_msg) =
                                    bincode::deserialize::<DhtPayload>(&packet.encrypted_payload)
                                {
                                    match dht_msg {
                                        DhtPayload::Ping => {
                                            tracing::info!(
                                                "    [+] PING recibido de {}. Respondiendo PONG.",
                                                src
                                            );
                                            let pong_bin =
                                ...
                                } else if let Ok(hs_payload) =
                                    bincode::deserialize::<transport::handshake::HandshakePayload>(
                                    ...
                                    )
                                {
                                    ...
                                    let shared_secret = local_session
                                        .derive_shared_secret(hs_payload.ephemeral_public_key);

                                    sessions_clone
                                        .add_secret(packet.source_id, shared_secret)
                                        .await;
                                        ...
                                        dht_clone.lookup(&packet.source_id).await
                                    {
                                        let _ = relay
                                            .send_raw_packet(
                                                &resp_packet.to_bytes().unwrap(),
                                                &target_addr,
                                            )
                                            .await;
                                    }
                                } else if let Ok(_hs_resp) = bincode::deserialize::<HandshakeResponse>(
                                    &packet.encrypted_payload,
                                ) {
                                    let _ = tx_clone
                                        .send(TuiEvent::LogMsg(
                                            "    [✓] Handshake RESPONSE Recibido. Túnel listo."
                                                .to_string(),
                                        ))
                                        .await;
                                } else if let Ok(relay_payload) =
                                    bincode::deserialize::<RelayInstruction>(
                                        &packet.encrypted_payload,
                                    )
                                {
                                    let _ = tx_clone
                                        .send(TuiEvent::LogMsg(
                                            "    [🛡️] Instrucción de RELÉ (Cebolla) detectada."
                                                .to_string(),
                                        ))
                                        .await;
                                    ...
                                    let target_addr = dht_clone
                                        .lookup(&relay_payload.target_id)
                                        .await
                                        .unwrap_or_else(|| "127.0.0.1:60553".to_string());
                                    tracing::info!(
                                        "[Cascada] Reenviando paquete pelado hacia: {}",
                                        target_addr
                                    );
                                    let _ = tx_clone
                                        .send(TuiEvent::LogMsg(format!(
                                            "    [🚀] Rebotando hacia IP física: {}",
                                            target_addr
                                        )))
                                        .await;
                                ...
                                let _ = tx_clone
                                    .send(TuiEvent::LogMsg(
                                        "    [X] ALERTA: Firma Inválida.".to_string(),
                                    ))
                                    .await;
                            }
                        }
                        Err(_) => {
                            let _ = tx_clone
                                .send(TuiEvent::LogMsg(
                                    "    [X] Basura irreconocible en puerto IPv7.".to_string(),
                                ))
                                .await;
                        }
                    }
        ...
        let decoded = bs58::decode(args[3].replace("id://", ""))
            .into_vec()
            .unwrap();
        ...
        tracing::info!(
            "[*] Ejecutando UDP NAT Punching hacia -> {}",
            target_address
        );
        let relay = OverlayRelay::start_listener("0.0.0.0").await?;
        ...
        relay
            .send_raw_packet(&p_packet.to_bytes().unwrap(), target_address)
            .await?;
        tracing::info!("[✓] PING enviado. Tu nodo ahora está registrado en la tabla del oponente.");

        // Esperando PONG
        let mut buf = [0u8; config::master::DEFAULT_PACKET_SIZE];
        if let Ok(Ok((_amt, _))) = tokio::time::timeout(
        ...
        let decoded = bs58::decode(args[2].replace("id://", ""))
            .into_vec()
            .unwrap();
        let mut target_pubkey = [0u8; 32];
        target_pubkey.copy_from_slice(&decoded[0..32]);
        ...
        tracing::info!(
            "[*] Consultando DHT local para localizar nodo id://{}...",
            args[2]
        );
        ...
        tracing::debug!(
            "[🚀] Empaquetado estructural ({} bytes en total).",
            raw_bytes.len()
        );
        tracing::info!(
            "[🚀] Disparando Handshake Request TCP-like sobre UDP hacia -> {}",
            target_address
        );
        ...
        let mut buf = [0u8; config::master::DEFAULT_PACKET_SIZE];
        if let Ok(Ok((amt, _))) = tokio::time::timeout(
            std::time::Duration::from_secs(3),
            relay.socket.recv_from(&mut buf),
        )
        .await
        {
            ...
            let hs_resp =
                bincode::deserialize::<HandshakeResponse>(&pkt.encrypted_payload).unwrap();
            let _shared_secret =
                handshake_session.derive_shared_secret(hs_resp.ephemeral_public_key);
        ...
        target_pubkey.copy_from_slice(
            &bs58::decode(args[2].replace("id://", ""))
                .into_vec()
                .unwrap()[0..32],
        );
        relay_pubkey.copy_from_slice(
            &bs58::decode(args[3].replace("id://", ""))
                .into_vec()
                .unwrap()[0..32],
        );
        ...
        tracing::info!(
            "    -> ¡Endpoint físico del Relé localizado! {}",
            relay_address
        );
        let target_address = dht
            .lookup(&target_pubkey)
            .await
            .unwrap_or_else(|| "127.0.0.1:60553".to_string());
        ...
        net_relay
            .send_raw_packet(&hs_packet.to_bytes().unwrap(), &target_address)
            .await?;
        ...
        net_relay
            .send_raw_packet(&final_raw_bytes, &relay_address)
            .await?;
        ...
        tracing::error!(
            "USO: ipv7-core [--listen] | [--connect] | [--cascade] | [--vpn] | [--ping] | [--say]"
        );
        wait_for_exit();
    }
